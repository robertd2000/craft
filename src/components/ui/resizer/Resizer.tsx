...
export const Resizer = ({ propKey, children, ...props }: any) => {
  ...
  const updateInternalDimensionsInPx = useCallback(() => {
    ...
    const width = percentToPx(
      ...
      (resizable?.current &&
        getElementDimensions(
          resizable?.current?.resizable?.parentElement as HTMLElement
        )?.width) ||
        0
    );
  ...
  const getUpdatedDimensions = (
    ...
    return {
      width: currentWidth + parseInt(width as string),
      height: currentHeight + parseInt(height as string),
    };
  };
    ...
    <Resizable
      ...
      ref={(ref) => {
        if (ref) {
          ...
          connect(resizable?.current?.resizable as HTMLElement);
        }
      }}
      ...
      onResize={(_, __, ___, d) => {
        ...
        if (isPercentage(nodeWidth))
          width =
            pxToPercent(
              width,
              getElementDimensions(dom?.parentElement as HTMLElement).width
            ) + "%";
        else width = `${width}px`;
